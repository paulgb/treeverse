...
export class Post {
  ...
  constructor(
    ...
    this.children = (thread.replies ?? []).map(
      (reply) => new Post(this, depth + 1, reply),
    );
    this.width = Math.max(
      1,
    ...
    this.height =
      this.children.reduce((acc, child) => Math.max(acc, child.height), 0) + 1;
  }
  ...
  private getChildrenInner(
    ...
    for (const child of this.children) {
      nodes.push(
        ...child.getChildrenInner(xOffset, yOffset + 1, xSize, ySize, thisNode),
      );
      xOffset += child.width;
    }
